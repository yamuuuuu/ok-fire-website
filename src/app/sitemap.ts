...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  const base = ['/', '/about', '/services', ...services.map(service => `/services/${service.slug}`), '/guides', ...fireGuides.map(guide => `/guides/${guide.slug}`), '/works', '/faq', '/contact', '/privacy'].map(url => ({ url: absoluteUrl(url), lastModified: new Date(), changeFrequency: url === '/' ? 'weekly' as const : 'monthly' as const, priority: url === '/' ? 1 : 0.7 }));
  try {
