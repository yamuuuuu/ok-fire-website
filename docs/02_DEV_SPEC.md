...
- 사진 첨부는 Phase 3에서 구현했으며 아래 17절의 검증·전송 방식을 따릅니다.

## 17. Phase 3 파일 구현 기준
- 저장소 미설정 시 사진 선택은 준비 안내로 표시합니다. 설정된 환경에서는 STEP 3에 선택/삭제/미리보기, STEP 5에 선택 사진 확인을 제공합니다.
- Vercel의 4.5MB 함수 본문 제한에 맞춰 JSON 예약 + 서명 PUT 직접 전송 + JSON 완료 검증으로 구현합니다. 파일당 10MiB 정책은 유지합니다.
- 브라우저 메모리에서 사진을 선택하고 개인정보 동의 및 접수 저장 후 전송합니다.
- 확장자/선언 MIME/실제 형식/크기/해시/디코딩 검증. 최대 5,000만 화소, 2,560px JPEG로 정규화하고 EXIF/GPS 제거.
- HEIC/HEIF 서버 디코딩 지원. 미리보기를 지원하지 않는 브라우저는 파일명과 선택 상태를 표시합니다.
- 소유 증명과 한시적 업로드 예약으로 접수 ID 추측만으로 파일을 올리는 행위를 막습니다.
- 정상 저장된 사진은 재전송해도 중복 생성하지 않습니다. 중단된 예약/미참조 파일 정리 명령을 제공합니다.
- 실제 저장소 계정/버킷은 아직 생성하지 않았으며 앱 환경변수와 운영 설정 절차는 루트 README를 따릅니다.
